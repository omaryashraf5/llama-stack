...
class OCIInferenceAdapter(OpenAIMixin):
    ...
    async def list_provider_model_ids(self) -> Iterable[str]:
        ...
        """
        logger.info("Starting list_provider_model_ids")
        try:
            oci_config = self._get_oci_config()
            oci_signer = self._get_oci_signer()
            compartment_id = self.config.oci_compartment_id or ""

            logger.info(f"Using compartment_id: {compartment_id}")
            ...
            # Don't filter by capability in the API call - the API doesn't accept "CHAT" as a valid capability
            # Instead, fetch all active models and filter locally for CHAT capability
            logger.info("Calling OCI list_models API")
            models: ModelCollection = client.list_models(
                compartment_id=compartment_id, lifecycle_state="ACTIVE"
            ).data

            logger.info(f"Received {len(models.items)} total models from OCI")

            seen_models = set()
            model_ids = []
            for model in models.items:
                logger.info(f"Processing model: {model.display_name}, capabilities: {model.capabilities}, deprecated: {model.time_deprecated}")

                if model.time_deprecated or model.time_on_demand_retired:
                    logger.info(f"  Skipping {model.display_name} - deprecated or retired")
                    continue

                if "CHAT" not in model.capabilities or "FINE_TUNE" in model.capabilities:
                    logger.info(f"  Skipping {model.display_name} - not CHAT or is FINE_TUNE")
                    continue

                # Use display_name + model_type as the key to avoid conflicts
                model_key = (model.display_name, ModelType.llm)
                if model_key in seen_models:
                    logger.info(f"  Skipping {model.display_name} - duplicate")
                    continue

                seen_models.add(model_key)
                model_ids.append(model.display_name)
                logger.info(f"  Added {model.display_name} to model list")

            logger.info(f"Returning {len(model_ids)} CHAT-capable models: {model_ids}")
            return model_ids
        except Exception as e:
            logger.error(f"Error in list_provider_model_ids: {e}", exc_info=True)
            raise
